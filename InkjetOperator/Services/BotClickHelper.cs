...
public static class BotClickHelper
{
    ...
    public static BotResult Run(string processName, BotStep[] steps)
    {
        ...
        for (int i = 0; i < steps.Length; i++)
        {
            ...
            if (!ClickAndVerify(step, i + 1)) // ✅ ส่ง stepIndex
                return BotResult.Fail($"{step.Name} failed");
    ...
    /// </summary>
    private static bool ClickAndVerify(BotStep step, int stepIndex)
    {
    ...
    private static void SaveStepCapture(Bitmap img, int stepIndex, string stepName)
    {
        var folder = Path.Combine(Application.StartupPath, "capture_log");
