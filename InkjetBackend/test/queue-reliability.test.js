...
const recoveryBody = (token, outcome = "sent") => ({ expected_token: token, request_id: randomUUID(),
  outcome, operator: "test operator", reason: "checked isolated fake printer", sender_stopped: true });
const recover = (id, body) => request(`/machine-queue/${id}/recover`, body);

test("recovery records audit once, keeps holder, and rejects late sender results", async () => {
  const { row } = await active(); const token = randomUUID();
  await begin(row.id, token); await finish(row.id, token, "unknown");
  const body = recoveryBody(token);
  const replies = await Promise.all([recover(row.id, body), recover(row.id, body)]);
  assert.ok(replies.every(r => r.status === 200));
  assert.equal(await PrintJobCommand.count({ where: { command: "MK", success: true } }), 1);
  const holder = await MachineQueue.findByPk(row.id);
  assert.equal(holder.state, "active"); assert.ok(holder.sent_at);
  const audit = await PrintJobCommand.findOne({ where: { command: "QUEUE_DISPATCH" } });
  assert.equal(audit.payload.recovery.operator, body.operator);
  assert.equal((await finish(row.id, token)).status, 409);
  assert.equal((await begin(row.id)).status, 409);
  assert.equal((await release(row.id)).status, 200);
});

test("crash recovery to not_sent requires explicit re-claim and rejects old snapshots", async () => {
  const { row } = await active(); const token = randomUUID();
  await begin(row.id, token);
  const body = recoveryBody(token, "not_sent");
  assert.equal((await recover(row.id, { ...body, sender_stopped: false })).status, 400);
  assert.equal((await recover(row.id, { ...body, operator: "" })).status, 400);
  assert.equal((await recover(row.id, { ...body, reason: "" })).status, 400);
  assert.equal((await recover(row.id, { ...body, expected_token: randomUUID() })).status, 409);
  assert.equal((await recover(row.id, body)).status, 200);
  assert.equal((await MachineQueue.findByPk(row.id)).state, "pending");
  assert.equal((await begin(row.id)).status, 409);
  assert.equal((await finish(row.id, token, "sent")).status, 409);
  assert.equal((await PrintJob.findByPk(row.print_jobs_id)).status, "Waiting");
  await claim(row.print_jobs_id);
  await begin(row.id);
  assert.equal((await recover(row.id, body)).status, 409);
});

test("competing recoveries cannot override each other and database failure rolls back", async () => {
  const { row } = await active(); const token = randomUUID(); await begin(row.id, token);
  MachineQueue.addHook("beforeUpdate", "fail-recovery", instance => {
    if (instance.changed("sent_at")) throw new Error("injected recovery failure");
  });
  try { assert.equal((await recover(row.id, recoveryBody(token))).status, 500); }
  finally { MachineQueue.removeHook("beforeUpdate", "fail-recovery"); }
  assert.equal(await PrintJobCommand.count({ where: { success: true } }), 0);
  assert.equal((await release(row.id)).status, 409);
  const replies = await Promise.all([recover(row.id, recoveryBody(token)), recover(row.id, recoveryBody(token, "not_sent"))]);
  assert.deepEqual(replies.map(r => r.status).sort(), [200, 409]);
});

test("recovery preserves another machine's unresolved send and races safely with finish", async () => {
  const j = await job(); await enqueue(j.id, [{ machine: "MK" }, { machine: "UV2" }]);
  const mk = (await claim(j.id, "MK")).data.claimed;
  const uv = (await claim(j.id, "UV2")).data.claimed;
  const a = randomUUID(), b = randomUUID();
  await begin(mk.id, a); await begin(uv.id, b);
  assert.equal((await recover(mk.id, recoveryBody(a, "not_sent"))).status, 200);
  assert.equal((await PrintJob.findByPk(j.id)).status, "Process");
  const replies = await Promise.all([finish(uv.id, b, "sent"), recover(uv.id, recoveryBody(b, "not_sent"))]);
  assert.deepEqual(replies.map(r => r.status).sort(), [200, 409]);
  const current = await MachineQueue.findByPk(uv.id);
  assert.ok((current.state === "active" && current.sent_at) || (current.state === "pending" && !current.sent_at));
});
